  ...
  window.AMEVA_PLUGINS["finance-dashboard"] = {
    ...
    render: function (containerId) {
      ...
      const fetchLiveFinance = async () => {
        ...
        try {
          const response = await fetch('http://localhost:3010/api/finance/data');
      ...
      const executeSearch = async () => {
        ...
        try {
          const response = await fetch(`http://localhost:3010/api/finance/search?q=${encodeURIComponent(query)}`);
          ...
          if (data && data.success && data.quotes && data.quotes.length > 0) {
            ...
            resultsContainer.querySelectorAll('.search-item').forEach(item => {
              item.addEventListener('click', async (e) => {
                ...
                try {
                  const detRes = await fetch(`http://localhost:3010/api/finance/stock-detail?ticker=${encodeURIComponent(ticker)}`);
